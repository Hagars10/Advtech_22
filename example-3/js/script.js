...
/**
  *this method makes it calculate how far to get to cursor, and start the trip
  *
  *Aparam evt the click event, to get cursor pos
  *
  */
function causeMoverTrip(evt){
    //make all browseres understand the event
    evt = (evt || event);
    //store the x position of the cursor
    var destination_x = evt.clientX;
    //calculate the overall distance the mover has to travel, subtracting the DIV's x from the mouse's x, converting DIV's X to an interager
    var mov = document.getElementsByTagName("div")[0];
    var x_distance = destination_x - parseInt(mov.style.left);
    //divide that distance by how many steps we want it to take. thats how much to move in one step:
